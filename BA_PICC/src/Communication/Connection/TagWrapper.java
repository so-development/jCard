...
public class TagWrapper implements TagTechnology {
	...
	public TagWrapper(Tag tag, String tech) {
		...
		try {
			...
			getMaxTransceiveLength = cls
					.getMethod("getMaxTransceiveLength");
			transceive = cls.getMethod("transceive",
					byte[].class);
	...
	public int getMaxTransceiveLength() {
		try {
			return (Integer) getMaxTransceiveLength
					.invoke(tagTech);
		} catch (IllegalArgumentException e) {
			throw new RuntimeException(e);
		} catch (IllegalAccessException e) {
